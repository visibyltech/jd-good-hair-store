...
export default function AdminLayout() {
  ...
  // Close menu when clicking outside on mobile or on route change
  useEffect(() => {
  ...
  return (
    <div className="admin-layout-container">
      ...
      <div className="admin-mobile-header" style={{ display: 'none', background: 'var(--card)', padding: '1rem', justifyContent: 'space-between', alignItems: 'center', zIndex: 20, borderBottom: '1px solid var(--border)' }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', color: 'var(--primary)', margin: 0 }}>
          Admin Panel
        </h2>
        <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--foreground)' }}>
      ...
      <aside className={`admin-sidebar ${mobileMenuOpen ? 'open' : ''}`}>
        <div style={{ marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <button 
            ...
            onClick={() => setMobileMenuOpen(false)} 
            style={{ display: 'none', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted-fg)' }}
          >
            <X size={24} />
          </button>
        </div>
